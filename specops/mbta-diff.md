...
15. Notice code count mismatch exists for `stop_too_far_from_shape`: old = 13, new = 12. ✅ Fixed: Python early-returned after first too-far stop; now continues to check all remaining stops.
16. Notice groups present only in old: `feed_expiration_date30_days` (1) and old-count version of `stop_too_far_from_shape` (13). ✅
17. Notice groups present only in new: `big_gap_in_service` (5), `feed_expiration_date_30_days` (1), `leading_or_trailing_whitespaces` (3), `mixed_case_recommended_field` (4), `service_has_no_active_day_of_the_week` (1), and new-count version of `stop_too_far_from_shape` (13). ❓
